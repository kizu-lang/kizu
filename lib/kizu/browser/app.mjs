// instantiate loads the app.wasm artifact emitted beside this module. It does
// not call start: loading a module and entering a Kizu program are separate,
// explicit operations.
export async function instantiate(options = {}) {
  const response = await fetch(new URL("./app.wasm", import.meta.url));
  if (!response.ok) {
    throw new Error(
      `failed to fetch Kizu WebAssembly: ${response.status} ${response.statusText}`.trimEnd(),
    );
  }
  return instantiateKizu(response, options);
}
...
export async function instantiateKizu(source, options = {}) {
  ...
  const imports = {
    kizu: {
      ...
      random(pointer, length) {
        // The host writes straight into guest memory, so the view is taken
        // for this import call only. getRandomValues fills at most 65536
        // bytes per call, so a longer draw is filled in slices.
        ...
        for (let at = 0; at < length; at += 65536) {
          source.getRandomValues(bytes.subarray(at, Math.min(at + 65536, length)));
        }
        return 0;
      },
    },

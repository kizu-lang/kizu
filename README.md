...
tags, so an example appears in more than one row. 186 runnable examples,
...
| `kizu check` | 186/186 |
| `kizu run` | 186/186 |
| `kizu build --emit-llvm` | 186/186 |
| `kizu build --target wasm32-wasi` (WAT) | 166/186 |
| `kizu build --target wasm32-wasi --opt` (WAT) | 166/186 |
| `kizu build --target wasm32-wasi --emit wasm -o <out>` | 166/186 |
| `kizu build --target wasm32-browser --emit wasm -o <out>` | 157/186 |
...
| wasm beyond the current target subsets | **in progress.** WASI WAT and binary routes run 166/186 runnable examples, browser binary runs 157/186, and all remaining cases are classified as target-unsupported capabilities |

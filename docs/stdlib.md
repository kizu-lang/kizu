...
| `std::json` | `encoder`, `begin_object`, `end_object`, `begin_array`, `end_array`, `begin_object_field`, `begin_array_field`, `write_i64`, `write_bool`, `write_null`, `write_bytes`, `write_*_field`, `finish_into`, `deinit` | Kizu source over `String`; API misuse traps through `std::internal::builtin::panic` | keep only the trap primitive trusted; `encode<T>` waits on comptime structural reflection |

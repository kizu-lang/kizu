# selfhost backend 一般化の台帳(履歴)

> **この文書は履歴です。** ADR-0081(2026-08-12)で自己コンパイル backend
> (`compiled_*` / `cli_*_llvm` / `llvm.kizu`、計 63,590 行)を削除したため、
> ここに並んでいる interception と shape 族は退役ではなく**消滅**しました。
> 新規記入はしません。残す理由は、次に backend を書き直すときに「何をやると
> こうなるか」の実例として読めるからです。
>
> 台帳が機能しなかった理由も ADR-0081 に記録してあります: 1 件 95 行の退役を
> 540 回繰り返す計画は、形状追加の速度に負けます。

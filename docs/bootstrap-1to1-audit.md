...
| parser | self-host AST counts, declaration order, declaration detail, selected control/expression/if-expression/qualified-call/error-return/match/stdlib-call/concurrency-call snapshots, and selected statement/expression AST node dump vs Go parser | weak | full AST node model and dump equality across all parseable conformance fixtures |

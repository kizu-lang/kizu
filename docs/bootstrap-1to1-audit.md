...
| diagnostics | self-host diagnostic objects cover lexer, parser, resolver missing/visibility, all `examples/negative/*.kizu` failure fixtures, module visibility/missing-module failures, selected positive counter-fixtures, static checker message/span snapshots, and runtime failure class snapshots vs Go facts | weak | add full positive conformance diagnostic-clean coverage and broaden runtime failure shape checks |
| type | self-host return/basic/control/variant and selected match/typed-error/unsafe/contract/Dyn/comptime/stdlib/concurrency/Io/fs/path/String/Map/Array resource/call/std-mem/task/channel/queue/thread/parallel boundary diagnostics plus local binding type environment snapshots for all parseable conformance manifest sources compare against Go facts | medium | broaden type-check snapshots to all checker diagnostic classes |

...
| diagnostics | self-host diagnostic objects cover lexer, parser, resolver missing/visibility, all `examples/negative/*.kizu` failure fixtures, module visibility/missing-module failures, static checker message/span snapshots, and runtime failure class snapshots vs Go facts; `TestSelfHostDiagnosticOracleCoversNegativeFixtures` fails if a negative fixture is not listed | strong | none |

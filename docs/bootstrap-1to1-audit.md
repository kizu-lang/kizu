...
| IR | self-host IR summary plus normalized function/block/opcode/result/operand/immediate/terminator dump for selected pass/fail fixtures, including call opcode/result/argument type rows, simple `try`/`error.try` order, struct literal/field read rows, selected `std::string::String` type-only namespace pass, and unsupported namespace/variant lowerability failures | weak | broaden to full instruction operand/value dump equality across conformance fixtures |

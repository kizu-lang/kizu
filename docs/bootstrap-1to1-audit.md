...
| diagnostics | self-host diagnostic objects cover lexer, parser, resolver missing/visibility, basic/control/variant and selected match/typed-error/unsafe/stdlib/concurrency/Io/fs/path/String/Map/Array resource type/call/std-mem/borrow, plus ownership move/borrow/mutable-borrow/field-borrow/deinit/Arena-Handle/String-view/Array-borrow spans vs Go facts | weak | broaden to all checker diagnostics and all conformance failure classes |
| type | self-host return/basic/control/variant and selected match/typed-error/unsafe/stdlib/concurrency/Io/fs/path/String/Map/Array resource/call/std-mem plus selected primitive/struct/stdlib local binding type snapshots compare pass/fail cases vs Go checker | weak | broaden to full type environment and all checker diagnostic classes |
| ownership | self-host owned-call/assignment move and borrow-while-move snapshots cover borrow, moved-value, double-move, assignment-move, deinit use-after-move, Arena/Handle provenance/escape, String view invalidation, Array append move/element-borrow conflicts, and move-while-borrowed fixtures vs Go checker | weak | full Kizu move/borrow checker with memory-safety diagnostic equality |

...
| ownership | 7893 | 12989 | 1.65(words 46,330 / 28,224 = 1.64) | check corpus 765 case(types が通った case の ownership diagnostics)+ unit(scope clone / merge) |
| ir | 5097 | 9177 | 1.80(words 32,826 / 17,509 = 1.87) | IR corpus 333 case(lower / opt の render)+ `TestSelfhostFrontend` の `tests/behavior` package `ir` / `ir --opt` byte 比較(std を含む CLI 経路)+ unit(verify の rejection) |
...
| compiler(cmd/kizu の 11 command すべて: parse / check / fmt / init / ir / build --emit-llvm / --target native / --target wasm32-wasi / run / test / cache / import-c-header / version) | 1035(全 command + `internal/version`) | 1723(main 1718 + import 衝突 helper 5。生成 file `version_source.kizu` は除く) | 1.66 | `TestSelfhostFrontend` が corpus / conformance に出ない各 command 境界の代表入力、init(生成 2 file、current directory、両 existing-file rejection)、CLI が引数を拒否する 13 の代表 command line(各 error message と、target を読めない 3 つの理由)、checker は通すが backend が拒否する `check`(`check: ok` が backend の受理まで含む約束であること)、`version` / `--version` を比較。run / test は `TestSelfhostBehavior`、cache は `TestSelfhostCache`、compiler/ 自身の fixed point は `TestSelfhostBootstrap` |

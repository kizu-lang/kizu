...
| error を catch する構文が未実装(`try` は伝播だけ) | buildcache の `readEntry` / `entries`: Go は壊れた entry JSON を miss / skip に落とし、並走 eviction で消えた entry の read 失敗も skip する | 壊れた entry は `std::json::decode_ignore_unknown` の失敗をそのまま伝播(Go: miss、selfhost: error)。eviction 競合は read 前の exists 検査で窓を狭める(閉じられない)。`catch` と set 合成は SPEC §11.1 / §11.2(ADR-0127)で採用済みで、実装後にこの 2 箇所を Go と同じ挙動に揃える |

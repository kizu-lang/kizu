...
既定が 1 なのは protocol ではなく server policy です。当初の理由は逐次 server で
1 connection が service 全体を保持することでした。TaskSet でその制約は外れましたが、
connection ごとの memory、idle timeout、公平性を測った新しい既定はまだ決めて
いません。閉じた 1 を維持し、`TODO.md` 0c で再検討します。
...
| 案 | 却下理由 |
| --- | --- |
| `max_requests` の既定を無制限にする(HTTP/1.1 の既定) | connection retention を無制限に開く policy になる。実測で有限の既定を選べるまでは、閉→開が additive な 1 を保つ(原理 8) |

...
| `fmt <source file>` | #648/#650 stdout-only formatter slice routed through the hosted selfhost formatter path; `--write` is out of scope | `KIZU_RUN_SELFHOST_GATES=1 go test -timeout=20m ./cmd/kizu -run TestSelfhostBackendArtifactGate -count=1 -v` |

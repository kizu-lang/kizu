...
| `fmt --write <source file>` | #1073 formatter mutation/no-mutation rows using the same selfhost formatted bytes and temp-write plus `fs_rename` publish | `just selfhost-fmt-parity-gate` |
...
comments, inline comments, and stable leading import sorting. Every successful

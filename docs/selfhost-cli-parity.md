...
`selfhost/tests/cli/parse-parity.tsv` is the #525/#579/#586/#594/#598 parse parity
...
positive print-call, positive `std::testing::expect(true|false)`, and negative
...
| broader `parse <file>` diagnostics | Partially deferred | #579, #586, #594, and #598 move the first positive, call-statement, qualified-call, and negative parse shapes off single hardcoded paths, but broader parsing and diagnostic recovery are not claimed. | Add one parse diagnostic/source shape at a time, keeping checked-in goldens and no Go fallback. |

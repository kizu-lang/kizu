...
| `fmt <source file>` | #648 stdout-only formatter slice routed through the hosted selfhost formatter path; `--write` is out of scope | `KIZU_RUN_SELFHOST_GATES=1 go test -timeout=20m ./cmd/kizu -run TestSelfhostBackendArtifactGate -count=1 -v` |
...
The hosted `fmt` slice accepts only the two-argument `fmt <file>` form; in-place
rewrite flags remain owned by the Go CLI until a separate issue defines hosted
mutation behavior.
...
| broader `fmt <file>` and `fmt --write` | Partially deferred | #648 adds only the first stdout formatter slice over the existing hosted formatter path; source-preserving comments, import ordering, and in-place writes are not claimed. | Extend formatter slices under #629 with explicit stdout/stderr, mutation, and comment-trivia contracts. |

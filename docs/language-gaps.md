...
| std collection の要素は `deinit(allocator)` を要求するので、allocator を使わない owner を入れられない | evented server が接続を `Array<net::TcpStream>` / `Arena<net::TcpStream>` に持てない。socket は descriptor を解放するので `deinit()` | `examples/net_poller.kizu` は接続を 3 つの名前付き変数で持つ。汎用の evented server はこれが直るまで書けない |

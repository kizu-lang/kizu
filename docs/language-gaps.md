...
| closure / captured callback が無いので state を捕捉する Go 関数(`collectModuleTypeNames(collect func)`、`writeContainerNew(isResultType func)`)をそのまま写せない | llvm の header / container | 呼び分けを enum(`TypeCollector`、`ContainerKind`)で受け、中で match する |
...
| `runtime.GOARCH` 相当が無い(OS は `std::target::is_darwin()` で答えが出る) | native の toolchainKey(cache key の host 部)と arm64 判定 | claim した temp dir で `sh -c "PATH=/usr/bin:/bin uname -sm > file"` を実行して読む(spawn の出力 capture 不能の既知 gap の続き)。未知の host は guess せず error で止める。target の選択は `stdtarget::host()` が comptime 述語で決め、uname は cache key にだけ使う |
...
| struct field への borrow を返す method が書けない(`&var self.field` も暗黙 borrow も型 error) | `std::http` の `Exchange` が接続を貸す口(`stream() -> &var net::TcpStream`)を書けない | 接続を field で公開せず、`write_all` / `read_into` / `set_*_deadline` の pass-through method を Exchange に置く。結果的に head 読みの残り byte を飛ばせなくなり、こちらの方が正しかった |
| expression の match arm で `?T` を作れない(`null` が arm に置けない) | `std::http` の `Response.framed_length(framing) -> ?i64` | `declares_length(framing) -> bool` と `length_of(framing) -> i64` の 2 つに割る。後者は到達しない arm を 0 で埋める |
| derived deinit は必ず allocator を渡すので、socket を field に持つ struct は自前 deinit が要る | `std::http::Exchange` が `TcpStream` を持つため deinit を手書きしている | 手書きのまま。container の element cleanup は `std::meta::release_names_allocator` で分岐するようになったので(ADR-0142)、同じ述語を `DeriveDeinit` にも通せば消える |

...
| 1 つの呼び出しで `self` の field を 2 つ(片方は `&var`)借りられない | `std::http` の `step_body` が inbox から `request.body` へ直接写せない | local に staging して 2 回に分ける。head は `max_head_bytes`、body は `max_body_bytes` で上限があるので copy も有界だが、1 回余計 |

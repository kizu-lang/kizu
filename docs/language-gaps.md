...
| expression の match arm で `?T` を作れない(`null` が arm に置けず、`let x: ?T =` の型注釈も無い) | `std::http` の `Response.framed_length(framing) -> ?i64` | `declares_length(framing) -> bool` と `length_of(framing) -> i64` の 2 つに割る。後者は到達しない arm を 0 で埋める |

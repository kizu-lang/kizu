...
func lexerSource(info lexerPackage) string {
	...
	out.WriteString("}\n")
	out.WriteString("\n")
	out.WriteString(lexerScanSource())
	...
	out.WriteString(lexerKeywordTypeSource())
...
// lexerScanSource returns cursor-carrying token scan helpers.
func lexerScanSource() string {
	return lexerScanStructSource() + lexerScanEntrySource() + lexerScanCoreSource() +
		lexerScanStringIdentSource() + lexerScanNumberPunctuationSource()
}

// lexerScanStructSource returns the scan result shape.
func lexerScanStructSource() string {
	return `pub struct TokenScan {
...
// lexerScanEntrySource returns public cursor scan entry points.
func lexerScanEntrySource() string {
	return `pub fn Scan(self: Lexer) -> TokenScan {
    return scan_token(self.input, self.position, self.line, self.column);
}

pub fn Advance(self: Lexer) -> Lexer {
    let scan = scan_token(self.input, self.position, self.line, self.column);
    return FromCursor(self.input, scan.next, scan.line, scan.column);
}

pub fn FromCursor(input: []const u8, position: i64, line: i64, column: i64) -> Lexer {
    return Lexer {
        input: input,
        position: position,
...
// lexerScanCoreSource returns the shared token scan implementation.
func lexerScanCoreSource() string {
	return `fn scan_token(input: []const u8, position: i64, line: i64, column: i64) -> TokenScan {
    ...
    if start >= length {
        return make_scan(token::EOFToken(), start, token_line, token_column, false);
    }
    if input[start] == cast<u8>(34) {
        return scan_string(input, start, length, token_line, token_column);
    }
    if is_letter(input[start]) {
        return scan_ident(input, start, length, token_line, token_column);
    }
    if is_digit(input[start]) {
        return scan_int(input, start, length, token_line, token_column);
    }
    return scan_punctuation(input, start, length, token_line, token_column);
}

`
}

// lexerScanStringIdentSource returns scan helpers for strings and identifiers.
func lexerScanStringIdentSource() string {
	return `fn scan_string(
    input: []const u8,
    start: i64,
    length: i64,
    token_line: i64,
    ...
    let end = string_end(input, start, length);
    if end >= length {
        return make_scan(
            make_token_at(token::Type::String, input[start + 1..length],
                token_line, token_column + 1),
            length,
            token_line,
            token_column + length - start,
            true
        );
    }
    return make_scan(
        make_token_at(token::Type::String, input[start + 1..end], token_line, token_column + 1),
        end + 1,
        token_line,
        token_column + end - start + 1,
        false
    );
}
...
fn scan_ident(
    ...
    let end = ident_end(input, start, length);
    let kind = keyword_type(input, start, end);
    let literal = input[start..end];
    return make_scan(
        make_token_at(kind, literal, token_line, token_column + 1),
        end,
        token_line,
        token_column + end - start,
        false
    );
}
...
// lexerScanNumberPunctuationSource returns scan helpers for remaining tokens.
func lexerScanNumberPunctuationSource() string {
	return `fn scan_int(
    input: []const u8,
    start: i64,
    length: i64,
    ...
    let end = digit_end(input, start, length);
    return make_scan(
        make_token_at(token::Type::Int, input[start..end], token_line, token_column + 1),
        end,
        token_line,
        token_column + end - start,
        false
...
fn scan_punctuation(
    ...
    let next = punctuation_end(input, start, length);
    let tok = punctuation_token_at(input, start, length, token_line, token_column + 1);
    let illegal = tok.Type == token::Type::Illegal;
    return make_scan(
        tok,
        next,
        token_line,
        token_column + next - start,
        illegal
    );
}
...
// lexerKeywordTypeSource returns byte-level keyword classification.
func lexerKeywordTypeSource() string {
	return `fn keyword_type(input: []const u8, start: i64, end: i64) -> token::Type {
    if keyword_code(input, start, end, 1) {
        return token::Type::Function;
    }
    if keyword_code(input, start, end, 2) {
        return token::Type::Import;
    }
    if keyword_code(input, start, end, 3) {
        return token::Type::Struct;
    }
    if keyword_code(input, start, end, 4) {
        return token::Type::Enum;
    }
    return token::LookupIdent(input[start..end]);
}
...
func lexerTokenAtSource() string {
    ...
    if is_letter(ch) {
        ...
        let kind = keyword_type(input, start, end);
        let literal = input[start..end];
        return make_token_at(kind, literal, token_line, token_column + 1);
    }
    if is_digit(ch) {
        var end = start;
        while end < length and is_digit(input[end]) {

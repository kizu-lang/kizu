...
func (e *emitter) writeHeader() {
	...
	// The probe is LLVM's own inline one on every target. On arm64 Darwin a
	// probe that calls the system __chkstk_darwin helper is one only Apple's
	// clang knows -- upstream LLVM stops at it with "Unsupported stack probing
	// method" -- and the inline one is written by both. LLVM 16 writes neither
	// there, which is one reason the development shell's clang is newer.

...
/* A std::arena::Arena<T> is that same header too. It owns its elements the way
 * an array does; what differs is above the runtime -- a Handle is an index
 * rather than a borrow, and nothing is removed from the middle -- so the arena
 * has no entry points here and reaches for the array ones. */

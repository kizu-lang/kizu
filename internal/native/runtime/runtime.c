...
static int64_t kizu_net_wait(int fd, short events, int64_t remaining) {
    int budget = -1;
    if (remaining >= 0) {
...
static KizuErrorI64 kizu_std_builtin_net_accept_result(
    ...
    int64_t deadline = kizu_net_deadline(timeout_ms);
    for (;;) {
        int64_t wait = kizu_net_wait((int)fd, POLLIN, kizu_net_remaining(deadline));
        if (wait == KIZU_NET_WAIT_RETRY) {
            continue;
        }
        if (wait != KIZU_NET_WAIT_READY) {
...
static KizuErrorVoid kizu_std_builtin_net_write_all_result(
    ...
    while (sent < bytes.len) {
        int64_t wait = kizu_net_wait((int)fd, POLLOUT, kizu_net_remaining(deadline));
        if (wait == KIZU_NET_WAIT_RETRY) {
            continue;
        }
        if (wait != KIZU_NET_WAIT_READY) {

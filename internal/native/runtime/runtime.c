...
_Static_assert(offsetof(KizuFsDirEntry, name) == 0,
               "KizuFsDirEntry.name leads the value");
_Static_assert(offsetof(KizuFsDirEntry, path) == 24,
               "KizuFsDirEntry.path follows the first String");
_Static_assert(offsetof(KizuFsDirEntry, is_dir) == 48,
               "KizuFsDirEntry.is_dir follows both Strings");
...
/* A map is an array of entries in the order they were first inserted, plus an
   ...
   An entry is this header followed by the value, rounded up to whole words, so
   an entry is KIZU_MAP_ENTRY_HEADER plus the value's words and a lookup that
   finds the key finds the value beside it. A key of at most eight bytes lives
   in the entry too; only a longer one is copied out to storage of its own. */
typedef struct {
    uint64_t hash;
    int64_t key_len;
    union {
...
#define KIZU_MAP_ENTRY_HEADER 24
_Static_assert(sizeof(KizuMapEntry) == KIZU_MAP_ENTRY_HEADER, "a map entry header is three words");
...
typedef struct {
    ...
    int64_t len;
    int64_t cap;
    /* index holds index_cap control bytes and then index_cap 32-bit entry
       numbers. A control byte is KIZU_MAP_FREE for a free slot, or the top
       seven bits of the hash of the entry the slot's number names, so a probe
       passes a slot whose hash differs by reading one byte -- and a lookup
       that misses reads no entry and no number at all, only a table a fifth
       the size of one of words. index_cap is a power of two and is kept above
       the entry count, so a probe always meets a free slot and terminates. */
    unsigned char *index;
    int64_t index_cap;
} KizuMap;
...
/* Random bytes come from the kernel's entropy source. getentropy answers at
   most 256 bytes per call and never returns short, so the draw is a loop of
   whole chunks straight into the String's storage; the length is committed
   only once every chunk was answered, so a refused draw leaves the String as
   it was. */
static KizuErrorVoid kizu_std_builtin_crypto_random_into_result(
    void *io, void *allocator, KizuString *dst, int64_t count) {
    if (kizu_io_is_failing(io)) {
        return kizu_err_void(KIZU_ERR_STD_CRYPTO_ERROR_IO_FAILING);
    }
    KizuArray *array = dst;
    if (!array) {
        return kizu_err_void(KIZU_ERR_STD_CRYPTO_ERROR_READ_FAILED);
    }
    if (count <= 0) {
        return kizu_ok_void();
    }
    if (!kizu_array_reserve_storage(allocator, array, array->len + count, 1)) {
        return kizu_err_void(KIZU_ERR_STD_CRYPTO_ERROR_OUT_OF_MEMORY);
    }
    int64_t filled = 0;
    while (filled < count) {
        ...
        if (getentropy(array->data + array->len + filled, want) != 0) {
            return kizu_err_void(KIZU_ERR_STD_CRYPTO_ERROR_READ_FAILED);
        }
        filled += (int64_t)want;
    }
...
void std__internal__builtin__crypto_random_into(
    KizuErrorVoid *out, void *io, void *allocator, KizuString *dst, int64_t count) {
    *out = kizu_std_builtin_crypto_random_into_result(io, allocator, dst, count);
}
...
static void kizu_sort_dir_entries(KizuArray *array) {
    ...
    for (int64_t index = 1; index < array->len; index++) {
        ...
        KizuSliceU8 item_name = {item.name.data, item.name.len};
        while (place >= 0) {
            KizuSliceU8 previous_name = {items[place].name.data, items[place].name.len};
            if (kizu_slice_compare(previous_name, item_name) <= 0) {
                break;
            }
...
static void kizu_fs_dir_entry_release(void *allocator, KizuFsDirEntry *entry) {
    if (!entry) {
        return;
    }
    kizu_rt_free(allocator, entry->name.data, entry->name.cap);
    kizu_rt_free(allocator, entry->path.data, entry->path.cap);
    entry->name = kizu_array_empty();
    entry->path = kizu_array_empty();
}

static void kizu_fs_dir_entries_release(void *allocator, KizuArray *array) {
    if (!array) {
        return;
    }
    KizuFsDirEntry *items = (KizuFsDirEntry *)array->data;
    for (int64_t index = 0; index < array->len; index++) {
        kizu_fs_dir_entry_release(allocator, &items[index]);
    }
    kizu_rt_free(
        allocator, array->data, array->cap * (int64_t)sizeof(KizuFsDirEntry));
    *array = kizu_array_empty();
}

static int kizu_fs_dir_entry_init(
    void *allocator, const char *dir, const char *name, KizuFsDirEntry *out) {
    size_t dir_len = strlen(dir);
    size_t name_len = strlen(name);
    if (dir_len > (uint64_t)INT64_MAX || name_len > (uint64_t)INT64_MAX) {
        return 0;
    }
    out->name = kizu_array_empty();
    out->path = kizu_array_empty();
    out->is_dir = 0;
    int needs_slash = dir_len > 0 && dir[dir_len - 1] != '/';
    if (!kizu_string_append_bytes(allocator, &out->name, name, (int64_t)name_len) ||
        !kizu_string_append_bytes(allocator, &out->path, dir, (int64_t)dir_len) ||
        (needs_slash && !kizu_string_append_bytes(allocator, &out->path, "/", 1)) ||
        !kizu_string_append_bytes(allocator, &out->path, name, (int64_t)name_len)) {
        kizu_fs_dir_entry_release(allocator, out);
        return 0;
    }
    return 1;
}

static KizuErrorArray kizu_std_builtin_fs_read_dir_result(
    void *io, void *allocator, KizuSliceU8 path) {
    if (kizu_io_is_failing(io)) {
    ...
    for (;;) {
        ...
        if (!entry) {
            if (errno != 0) {
                failure = kizu_errno_failure(errno);
            }
            break;
        }
        ...
        KizuFsDirEntry item;
        if (!kizu_fs_dir_entry_init(allocator, cpath, entry->d_name, &item)) {
            failure = KIZU_ERR_STD_FS_ERROR_OUT_OF_MEMORY;
            break;
        }
        KizuSliceU8 item_path = {item.path.data, item.path.len};
        char *entry_path = kizu_slice_to_cstr(item_path);
        if (!entry_path) {
            kizu_fs_dir_entry_release(allocator, &item);
            failure = KIZU_ERR_STD_FS_ERROR_OUT_OF_MEMORY;
            break;
        }
        struct stat st;
        ...
        free(entry_path);
        if (!kizu_array_append(
                allocator, &array, &item, (int64_t)sizeof(KizuFsDirEntry))) {
            kizu_fs_dir_entry_release(allocator, &item);
            failure = KIZU_ERR_STD_FS_ERROR_OUT_OF_MEMORY;
            break;
        }
    ...
    int close_code = errno;
    free(cpath);
    if (failure == 0 && close_result != 0) {
        failure = kizu_errno_failure(close_code);
    }
    if (failure != 0) {
        kizu_fs_dir_entries_release(allocator, &array);
        return kizu_err_array(failure);
    }
    kizu_sort_dir_entries(&array);
    return kizu_ok_array(array);
}

void std__internal__builtin__fs_read_dir(
    KizuErrorArray *out, void *io, void *allocator, const KizuSliceU8 *path) {
    *out = kizu_std_builtin_fs_read_dir_result(io, allocator, *path);
}
...
typedef struct {
    KizuPool *pool;
    /* Which scratch run the helper gathers lanes into; the caller has 0. */
    int64_t participant;
    pthread_t id;
    KizuStack stack;
} KizuPoolHelper;
...
struct KizuPool {
    ...
    int64_t len;
    int64_t elem_size;
    /* A chunk round cuts `chunk` elements at a time and has stride 0. A lane
       round has stride >= 1: lane k is `length` elements `stride` apart, and
       a stride above 1 is gathered into the taking thread's scratch. */
    int64_t chunk;
    int64_t length;
    int64_t stride;
    ...
    int64_t done;
    /* The first failure a worker reported this round, 0 while none has. */
    int64_t failure;
...
/* Runs one lane. A lane whose elements sit side by side is handed over where
   ...
   and no other thread holds any of its elements. */
static int64_t kizu_pool_run_lane(KizuPool *pool, int64_t participant, int64_t index) {
    int64_t block = index / pool->stride;
    int64_t lane = index % pool->stride;
    int64_t size = pool->elem_size;
    unsigned char *first =
...
static int64_t kizu_pool_run_item(KizuPool *pool, int64_t participant, int64_t index) {
    if (pool->stride > 0) {
        return kizu_pool_run_lane(pool, participant, index);
    }
...
/* Records the round's first failure and takes every batch nobody has taken
   yet, so no thread starts one; answers how many items those batches held.
   Batches already taken run to their end. */
static int64_t kizu_pool_fail(KizuPool *pool, uint64_t tag, int64_t failure, int64_t items) {
    int64_t none = 0;
    __atomic_compare_exchange_n(
        &pool->failure, &none, failure, 0, __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE);
    ...
    for (;;) {
        ...
        if (__atomic_compare_exchange_n(
                &pool->ticket, &ticket, kizu_pool_ticket(tag, batches, batches), 1,
                __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE)) {
            int64_t taken = (int64_t)next * pool->batch;
            return taken < items ? items - taken : 0;
        }
    }
}
...
static void kizu_pool_run_items(KizuPool *pool, int64_t participant, uint64_t tag) {
    ...
    for (;;) {
        ...
        int64_t finished = end - first;
        for (int64_t index = first; index < end; index++) {
            if (__atomic_load_n(&pool->failure, __ATOMIC_RELAXED) != 0) {
                break;
            }
            int64_t failure = kizu_pool_run_item(pool, participant, index);
            if (failure != 0) {
                finished += kizu_pool_fail(pool, tag, failure, items);
                break;
            }
        }
        int64_t done = __atomic_add_fetch(&pool->done, finished, __ATOMIC_ACQ_REL);
        if (done == items) {
...
/* A round is often shorter than the time a sleeping thread takes to wake, so
   a thread that runs out of work watches for the next change a little while
   before it sleeps. */
#define KIZU_POOL_SPINS 20000

static inline void kizu_pool_pause(void) {
...
/* Starts the round the caller has written, runs items on the caller too, and
   returns once every item is done or skipped after a failure: the first
   failure a worker reported, or 0. */
static int64_t kizu_pool_round(KizuPool *pool) {
    if (pool->items <= 0) {
        return 0;
    }
    ...
    __atomic_store_n(&pool->failure, 0, __ATOMIC_RELAXED);
    ...
    for (int spin = 0; spin < KIZU_POOL_SPINS; spin++) {
        if (__atomic_load_n(&pool->done, __ATOMIC_ACQUIRE) == pool->items) {
            return __atomic_load_n(&pool->failure, __ATOMIC_ACQUIRE);
        }
        kizu_pool_pause();
    }
    pthread_mutex_lock(&pool->lock);
...
void std__internal__builtin__thread_pool_new(
    ...
    for (int64_t i = 0; i < helper_count; i++) {
        ...
        pool->helpers[i].participant = i + 1;
...
void std__internal__builtin__thread_pool_each(
    ...
    *out = kizu_ok_void();
    ...
    pool->stride = 0;
    pool->scratch = NULL;
    pool->items = (data->len + chunk - 1) / chunk;
    int64_t failure = kizu_pool_round(pool);
    if (failure != 0) {
        *out = kizu_err_void(failure);
    }
}

/* Runs one lane round. `data` is whole blocks of length * stride elements;
...
void std__internal__builtin__thread_pool_each_lane(
    ...
    *out = kizu_ok_void();
    ...
    pool->scratch = NULL;
    if (scratch) {
        kizu_rt_free(allocator, scratch, scratch_bytes);
    }
    if (failure != 0) {
        *out = kizu_err_void(failure);
    }
...
typedef struct KizuTaskSet {
    ...
    size_t task_count;
    size_t task_cap;
    /* How many workers have left the set over its life. A wait for one to
       leave watches this rather than the count, which a spawn meanwhile would
       push back up. */
    uint64_t departed;
} KizuTaskSet;
...
int64_t std__internal__builtin__task_set_running(int64_t handle) {
    KizuTaskSet *tasks = kizu_task_set_from(handle);
    if (!tasks) {
        return 0;
    }
    return (int64_t)tasks->task_count;
}

/* Runs the loop until one worker has left the set. This is what lets a loop
   that spawns keep its number of workers bounded: at the cap it waits for one
   to finish instead of refusing or dropping (ADR-0148). The loop turns from
   wherever the caller stands, the way await does, so everyone else runs
   meanwhile. A cancelled caller is told so rather than made to wait on. An
   empty set, or a blocking Io whose workers already ran to their end inside
   spawn, has nothing to wait for. */
void std__internal__builtin__task_set_wait_one(
    KizuErrorVoid *out, void *io, int64_t handle) {
    KizuTaskSet *tasks = kizu_task_set_from(handle);
    KizuLoop *loop = kizu_io_loop(io);
    if (!tasks || !loop || tasks->task_count == 0) {
        *out = kizu_ok_void();
        return;
    }
    KizuCoro *caller = kizu_coro_current;
    uint64_t seen = tasks->departed;
    while (tasks->departed == seen) {
        if (caller && caller->canceled) {
            *out = kizu_err_void(KIZU_ERR_STD_IO_ERROR_CANCELED);
            return;
        }
        if (!kizu_loop_turn(loop)) {
            break;
...
// Half-closes the writing side: the peer reads end-of-stream after what was
// already written, and can still send. A descriptor that is not connected any
// more has nothing to half-close, and nothing to report about it.
void std__internal__builtin__net_shutdown_write(int64_t fd) {
    if (fd >= 0) {
...
#define KIZU_MAP_FREE 0x80
#define KIZU_MAP_SLOT_BYTES 5

static unsigned char kizu_map_tag(uint64_t hash) {
    return (unsigned char)(hash >> 57);
}

static uint32_t *kizu_map_numbers(const KizuMap *map) {
    /* index_cap is a power of two of at least eight, so the numbers start on a
       32-bit boundary. */
    return (uint32_t *)(void *)(map->index + map->index_cap);
}

/* kizu_map_entry_size is the width of one entry of a map whose values are
   value_size bytes. */
static int64_t kizu_map_entry_size(int64_t value_size) {
    return KIZU_MAP_ENTRY_HEADER + ((value_size + 7) & ~(int64_t)7);
}

static KizuMapEntry *kizu_map_entry(const KizuMap *map, int64_t number, int64_t entry_size) {
    return (KizuMapEntry *)(map->entries + number * entry_size);
}

static const unsigned char *kizu_map_entry_key(const KizuMapEntry *entry) {
    return entry->key_len <= 8 ? entry->key.bytes : entry->key.copy;
}

static unsigned char *kizu_map_entry_value(KizuMapEntry *entry) {
    return (unsigned char *)entry + KIZU_MAP_ENTRY_HEADER;
}

/* kizu_map_short_key reads a key of at most eight bytes as the zero-padded
   word an entry stores it as, so comparing keys is comparing words. */
static uint64_t kizu_map_short_key(const unsigned char *key, int64_t key_len) {
    uint64_t word = 0;
    if (key_len == 8) {
        memcpy(&word, key, 8);
        return word;
    ...
    for (int64_t i = 0; i < key_len; i += 1) {
        word |= (uint64_t)key[i] << (8 * i);
    }
    return word;
}

/* kizu_map_long_key_equal compares a key longer than eight bytes with the copy
   an entry holds. It stays out of line, so the probe around it keeps no
   registers across a call it rarely makes. */
__attribute__((noinline))
static _Bool kizu_map_long_key_equal(
    const KizuMapEntry *entry, const unsigned char *key, int64_t key_len) {
    return memcmp(entry->key.copy, key, (size_t)key_len) == 0;
}
...
static inline int64_t kizu_map_slot(
    KizuMap *map, const unsigned char *key, int64_t key_len, uint64_t hash, int64_t entry_size) {
    uint64_t mask = (uint64_t)map->index_cap - 1;
    uint64_t slot = hash & mask;
    unsigned char tag = kizu_map_tag(hash);
    _Bool is_short = key_len <= 8;
    uint64_t short_key = is_short ? kizu_map_short_key(key, key_len) : 0;
    for (;;) {
        unsigned char control = map->index[slot];
        if (control == KIZU_MAP_FREE) {
            return (int64_t)slot;
        }
        if (control == tag) {
            KizuMapEntry *entry = kizu_map_entry(map, kizu_map_numbers(map)[slot], entry_size);
            if (entry->hash == hash && entry->key_len == key_len) {
                if (is_short) {
                    uint64_t stored;
                    memcpy(&stored, entry->key.bytes, 8);
                    if (stored == short_key) {
                        return (int64_t)slot;
                    }
                } else if (kizu_map_long_key_equal(entry, key, key_len)) {
                    return (int64_t)slot;
                }
            }
        }
...
/* kizu_map_find returns the number of the entry holding key, or -1. */
__attribute__((always_inline))
static inline int64_t kizu_map_find(
    KizuMap *map, const unsigned char *key, int64_t key_len, int64_t entry_size) {
    if (!map || (!key && key_len > 0) || key_len < 0 || map->index_cap == 0) {
        return -1;
    }
    uint64_t hash = kizu_map_hash(key, key_len);
    int64_t slot = kizu_map_slot(map, key, key_len, hash, entry_size);
    return map->index[slot] == KIZU_MAP_FREE ? -1 : (int64_t)kizu_map_numbers(map)[slot];
}

/* kizu_map_place_entries files every entry into the index by its stored hash.
   The index is expected empty; the probe stops at the free slot it is bound
   to meet. */
static void kizu_map_place_entries(KizuMap *map, int64_t entry_size) {
    uint64_t mask = (uint64_t)map->index_cap - 1;
    uint32_t *numbers = kizu_map_numbers(map);
    for (int64_t number = 0; number < map->len; number += 1) {
        uint64_t hash = kizu_map_entry(map, number, entry_size)->hash;
        uint64_t slot = hash & mask;
        while (map->index[slot] != KIZU_MAP_FREE) {
            slot = (slot + 1) & mask;
        }
        map->index[slot] = kizu_map_tag(hash);
        numbers[slot] = (uint32_t)number;
    }
}
...
/* kizu_map_reindex grows the index to hold needed entries below a 3/4 load and
   ...
   The first index holds eight slots rather than sixteen: the compiler keeps
...
static _Bool kizu_map_reindex(void *allocator, KizuMap *map, int64_t needed, int64_t entry_size) {
    if (needed > (int64_t)UINT32_MAX) {
        return 0;
    }
    ...
    unsigned char *index = (unsigned char *)kizu_rt_alloc(allocator, next * KIZU_MAP_SLOT_BYTES);
    if (!index) {
        return 0;
    }
    memset(index, KIZU_MAP_FREE, (size_t)next);
    kizu_rt_free(allocator, map->index, map->index_cap * KIZU_MAP_SLOT_BYTES);
    map->index = index;
    map->index_cap = next;
    kizu_map_place_entries(map, entry_size);
    return 1;
}

/* kizu_map_remove moves the value stored for key into out and drops the
   entry: a key copy goes back to the allocator, the entries behind it close
   the gap so insertion order holds (ADR-0088), and the index is rebuilt in
   place, since every entry behind the gap changed number. */
_Bool kizu_map_remove(
    void *allocator, KizuMap *map, const unsigned char *key, int64_t key_len,
    void *out, int64_t value_size) {
    if (!out || value_size <= 0) {
        return 0;
    }
    int64_t entry_size = kizu_map_entry_size(value_size);
    int64_t found = kizu_map_find(map, key, key_len, entry_size);
    if (found < 0) {
        return 0;
    }
    KizuMapEntry *entry = kizu_map_entry(map, found, entry_size);
    memcpy(out, kizu_map_entry_value(entry), (size_t)value_size);
    if (entry->key_len > 8) {
        kizu_rt_free(allocator, entry->key.copy, entry->key_len);
    }
    memmove(entry, (unsigned char *)entry + entry_size,
        (size_t)((map->len - found - 1) * entry_size));
    map->len -= 1;
    memset(map->index, KIZU_MAP_FREE, (size_t)map->index_cap);
    kizu_map_place_entries(map, entry_size);
    return 1;
}

static _Bool kizu_map_reserve(void *allocator, KizuMap *map, int64_t needed, int64_t entry_size) {
    if (needed <= map->cap) {
        return 1;
    }
    ...
    if (next > INT64_MAX / entry_size) {
        return 0;
    }
    unsigned char *entries = (unsigned char *)kizu_rt_realloc(
        allocator, map->entries, map->cap * entry_size, next * entry_size);
    if (!entries) {
        return 0;
    }
    memset(entries + map->cap * entry_size, 0, (size_t)((next - map->cap) * entry_size));
    map->entries = entries;
    map->cap = next;
    return 1;
}
...
_Bool kizu_map_insert(
    ...
    if (!kizu_map_reindex(allocator, map, map->len + 1, entry_size)) {
        return 0;
    }
    uint64_t hash = kizu_map_hash(key, key_len);
    int64_t slot = kizu_map_slot(map, key, key_len, hash, entry_size);
    if (map->index[slot] != KIZU_MAP_FREE) {
        KizuMapEntry *entry = kizu_map_entry(map, kizu_map_numbers(map)[slot], entry_size);
        memcpy(kizu_map_entry_value(entry), value, (size_t)value_size);
        return 1;
    }
    /* The key is taken into the entry's own form before the entries can move:
       a key may be read out of this map's storage. */
    KizuMapEntry stored;
    stored.hash = hash;
    stored.key_len = key_len;
    if (key_len <= 8) {
        uint64_t word = kizu_map_short_key(key, key_len);
        memcpy(stored.key.bytes, &word, 8);
    } else {
        stored.key.copy = (unsigned char *)kizu_rt_alloc(allocator, key_len);
        if (!stored.key.copy) {
            return 0;
        }
        memcpy(stored.key.copy, key, (size_t)key_len);
    }
    if (!kizu_map_reserve(allocator, map, map->len + 1, entry_size)) {
        if (key_len > 8) {
            kizu_rt_free(allocator, stored.key.copy, key_len);
        }
        return 0;
    }
    KizuMapEntry *entry = kizu_map_entry(map, map->len, entry_size);
    *entry = stored;
    memcpy(kizu_map_entry_value(entry), value, (size_t)value_size);
    map->index[slot] = kizu_map_tag(hash);
    kizu_map_numbers(map)[slot] = (uint32_t)map->len;
    map->len += 1;
    return 1;
}

/* kizu_map_get_word is kizu_map_get for an eight-byte key, the width of
   every word-sized key the compiler hands over. With the width fixed, the hash
   is one mix of the word and the probe compares words, and nothing it does
   calls out, so it keeps no registers across a call. */
__attribute__((noinline))
static void *kizu_map_get_word(KizuMap *map, const unsigned char *key, int64_t value_size) {
    if (map->index_cap == 0) {
        return NULL;
    }
    uint64_t word;
    memcpy(&word, key, 8);
    uint64_t hash = ((uint64_t)8 << 5 ^ word) * 0x517cc1b727220a95ULL;
    hash ^= hash >> 32;
    hash *= 0xd6e8feb86659fd93ULL;
    hash ^= hash >> 32;
    int64_t entry_size = kizu_map_entry_size(value_size);
    ...
    for (;;) {
        ...
        if (control == tag) {
            ...
            uint64_t stored;
            memcpy(&stored, entry->key.bytes, 8);
            if (entry->hash == hash && entry->key_len == 8 && stored == word) {
                return kizu_map_entry_value(entry);
            }
        }
        slot = (slot + 1) & mask;
    }
...
void *kizu_map_get(KizuMap *map, const unsigned char *key, int64_t key_len, int64_t value_size) {
    if (map && key && key_len == 8) {
        return kizu_map_get_word(map, key, value_size);
    }
    int64_t entry_size = kizu_map_entry_size(value_size);
    int64_t found = kizu_map_find(map, key, key_len, entry_size);
    if (found < 0) {
    ...
    return kizu_map_entry_value(kizu_map_entry(map, found, entry_size));
}
...
void *kizu_map_value_at(KizuMap *map, int64_t index, int64_t value_size) {
    if (!map || index < 0 || index >= map->len) {
        return NULL;
    }
    return kizu_map_entry_value(kizu_map_entry(map, index, kizu_map_entry_size(value_size)));
}

void kizu_map_key_at(KizuOptSliceU8 *out, KizuMap *map, int64_t index, int64_t value_size) {
    if (!map || index < 0 || index >= map->len) {
        *out = kizu_opt_null_slice();
        return;
    }
    KizuMapEntry *entry = kizu_map_entry(map, index, kizu_map_entry_size(value_size));
    KizuSliceU8 key;
    key.ptr = (unsigned char *)kizu_map_entry_key(entry);
    key.len = entry->key_len;
    *out = kizu_opt_slice(key);
}

_Bool kizu_map_contains(KizuMap *map, const unsigned char *key, int64_t key_len, int64_t value_size) {
    return kizu_map_find(map, key, key_len, kizu_map_entry_size(value_size)) >= 0;
}
...
void kizu_map_deinit(void *allocator, KizuMap *map, int64_t value_size) {
    ...
    for (int64_t i = 0; i < map->len; i += 1) {
        KizuMapEntry *entry = kizu_map_entry(map, i, entry_size);
        if (entry->key_len > 8) {
            kizu_rt_free(allocator, entry->key.copy, entry->key_len);
        }
    }
    kizu_rt_free(allocator, map->entries, map->cap * entry_size);
    kizu_rt_free(allocator, map->index, map->index_cap * KIZU_MAP_SLOT_BYTES);
}

...
static KizuErrorI64 kizu_std_builtin_net_accept_result(void *io, int64_t fd) {
    if (kizu_io_is_failing(io)) {
        return kizu_err_i64(KIZU_ERR_STD_NET_ERROR_IO_FAILING);
    }
    if (fd < 0) {
...
void std__internal__builtin__net_accept(KizuErrorI64 *out, void *io, int64_t fd) {
    *out = kizu_std_builtin_net_accept_result(io, fd);
}
...
void std__internal__builtin__net_read(
    ...
    *out = kizu_std_builtin_net_read_result(io, allocator, fd, dst, max);
}
...
void std__internal__builtin__net_write_all(
    ...
    *out = kizu_std_builtin_net_write_all_result(io, fd, *bytes);
}

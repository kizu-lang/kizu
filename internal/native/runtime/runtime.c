 ...
 * With no deadline it does not poll at all -- the blocking call that follows
 * is the wait -- so a stream without one pays nothing for this.
 *
...
static KizuErrorVoid kizu_std_builtin_net_write_all_result(
    ...
    while (sent < bytes.len) {
        int64_t wait = kizu_net_wait(io, (int)fd, POLLOUT, kizu_net_remaining(deadline));
        if (wait == KIZU_NET_WAIT_RETRY) {
            continue;
        }
        if (wait != KIZU_NET_WAIT_READY) {
         ...
         * poll above -- and the deadline with it -- would never be read again.
         *
        ...
        if (wrote < 0) {
            ...
            if (kizu_net_would_block(errno)) {
                /* The room poll saw is gone. Poll again, which is where the
                 * deadline is read. */
                continue;
            }

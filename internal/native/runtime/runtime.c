...
static _Bool kizu_arena_reserve(KizuArena *arena, int64_t needed) {
    ...
    /* Grow the way an array does rather than by doubling: a factor below the
       golden ratio lets a later allocation reuse blocks freed by earlier ones,
       and an arena holding the AST and the IR is the largest thing the
       compiler keeps, so the half it would otherwise carry unused is real. */
    int64_t next = kizu_array_grow_capacity(needed, arena->elem_size);

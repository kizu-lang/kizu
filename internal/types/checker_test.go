...
// TestCheckRuntimeCaptureCapturesExactTypes checks the one inference exception:
// trailing arguments bring their standalone types into each comptime expansion.
func TestCheckRuntimeCaptureCapturesExactTypes(t *testing.T) {
	source := `fn accept(parts: ...) -> void {
...
fn main() -> void { accept(1, true, 2); }`
	if err := checkSource(source); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
}

// TestCheckRuntimeCaptureAcceptsBorrow checks that capture preserves an
// ordinary fixed parameter's shared-borrow mode instead of excluding its type.
func TestCheckRuntimeCaptureAcceptsBorrow(t *testing.T) {
	source := `fn take(parts: ...) -> void {
    comptime for parts |T, part| { print(part); }
    return;
}
fn main() -> void { let value = 1; take(&value); }`
	if err := checkSource(source); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
}

// TestCheckRuntimeCaptureIsNotAValue points authors to its one opening form.
func TestCheckRuntimeCaptureIsNotAValue(t *testing.T) {
	source := `fn take(parts: ...) -> void {
    print(parts);
    return;
}
fn main() -> void { take(1); }`
	...
	if err == nil || !strings.Contains(err.Error(),
		"open it with `comptime for parts |T, value|`") {
		t.Fatalf("error = %v, want comptime-for guidance", err)
	}
}

// TestCheckRuntimeCaptureNeedsDistinctBindings keeps its type and value usable.
func TestCheckRuntimeCaptureNeedsDistinctBindings(t *testing.T) {
	source := `fn take(parts: ...) -> void {
    comptime for parts |part, part| { print(part); }
    return;
}
fn main() -> void { take(1); }`
	err := checkSource(source)
	if err == nil || !strings.Contains(err.Error(), "must use different names") {
		t.Fatalf("error = %v, want distinct capture guidance", err)
	}
}

// TestCheckFmtAcceptsUserDisplayMethod keeps display extensible through an
// ordinary statically dispatched method rather than a closed type whitelist.
func TestCheckFmtAcceptsUserDisplayMethod(t *testing.T) {
	source := `struct User { name: []u8 }
fn (self: &User) append_display(out: &var std::string::String) -> !void {
    return out.append_bytes(self.name);
}
fn main() -> !void {
    let allocator = std::mem::page_allocator();
    let user = User { name: "Aki" };
    let text = try std::fmt::format(allocator, "user=", &user);
    defer text.deinit();
    return;
}`
	if err := checkSource(source); err != nil {
...
// TestCheckRuntimeCaptureLengthBound gives generated source a finite expansion boundary.
func TestCheckRuntimeCaptureLengthBound(t *testing.T) {
	args := strings.TrimSuffix(strings.Repeat("1, ", 65), ", ")
	source := `fn take(parts: ...) -> void {
    comptime for parts |T, part| { print(part); }
    return;
}
fn main() -> void { take(` + args + `); }`
	err := checkSource(source)
	if err == nil || !strings.Contains(err.Error(), "maximum is 64") {
		t.Fatalf("error = %v, want runtime capture length bound", err)
	}
}

// TestCheckFallibleRuntimeCaptureAcceptsOwner lets each expansion name and explicitly
// clean the fixed owner parameter on both success and error paths.
func TestCheckFallibleRuntimeCaptureAcceptsOwner(t *testing.T) {
	source := `fn take(parts: ...) -> !void {
    comptime for parts |T, part| {
...
// TestCheckRejectsRuntimeCaptureOutsideFreeBody keeps the surface to one
// trailing capture on a body-bearing free function.
func TestCheckRejectsRuntimeCaptureOutsideFreeBody(t *testing.T) {
	cases := []struct {
		name   string
		source string
		want   string
		...
		{
			name: "method",
			source: `struct Item { value: i64 }
fn (self: &Item) inspect(parts: ...) -> void {
    comptime for parts |T, part| { print(part); }
    return;
}`,
			want: "only on free functions",
		},
		{
			name: "contract requirement",
			source: `contract Inspect {
    fn inspect(parts: ...) -> void;
}`,
			want: "require a function body",
		},
		{
			name:   "extern function",
			source: `extern "c" fn inspect(parts: ...) -> void`,
			want:   "extern function `inspect` cannot declare",
		},
		{
			name: `duplicate fixed name`,
			source: `fn inspect(parts: i64, parts: ...) -> void {
    comptime for parts |T, part| { print(part); }
    return;
}`,
			want: "duplicates a parameter",
		},
		{
			name: `duplicate static name`,
			source: `fn inspect<parts>(parts: ...) -> void {
    comptime for parts |T, part| { print(part); }
    return;
}`,
			want: "duplicates a static parameter",
		},
	}

...
// TestSelfhostProductionBoundaryGate runs #458 commands through the stage2 artifact.
...
func productionBoundaryText(key string) string {
	...
	case "check-ok":
		return "check: ok\n"
	case "stage-ok":
		return bootstrapStageStdout()
	case "unsupported-command":
		return "unsupported selfhost command\n"
	default:
...
func appendProductionBoundaryHeader(out *strings.Builder) {
	...
	fmt.Fprintf(out, "production.path hosted-stage2-artifact\n")

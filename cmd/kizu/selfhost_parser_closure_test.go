...
var parserClosureSeeds = []string{
	...
	"synth_postfix_loop",
	// issue 1157 NEW-A fixture: synth_child_assembly opens a child range, appends one NodeId with
	// ast.add_child, and closes it with ast.finish_children. It lowers the child-array mutator method
	// calls + bodies in isolation, replacing the parse_primary seed, which pulled the whole recursive
	// expression SCC. add_call, the i64 drain, and the value-carried append loop are deferred.

...
`f32` と `f64` は IEEE 754 の binary32 / binary64 です。二項演算子は `+ - * /`、
単項は `-`、比較は `== != < <= > >=` で、両辺は同じ float 型でなければなりません。
`%` と bit 演算は整数だけのものです。整数と float を混ぜる演算はなく、
`x * 2` は `x` が float なら error です(`x * 2.0` と書く)。

演算と比較は IEEE 754 のとおりです。0 で割ると無限大か NaN になり、trap
しません。NaN は自分自身を含む何とも等しくなく、`!=` だけが true です。

`cast<T>(value)` は数値どうしの変換で、意味は次のとおりです。

* 整数 → float: 最も近い値に丸める
* float → 整数: 0 に向かって切り捨て、範囲を超える値は型の最小値・最大値に
  飽和し、NaN は 0 になる。poison や未定義の値は生まれない
* `f64` → `f32`: 最も近い値に丸める。`f32` → `f64` は正確

float literal は `1.0`、`2.5e-3`、`1e6` の形で、`.` の両側と指数に数字が要ります
(`1.` や `.5` は literal ではない)。型は文脈が `f32` を求めるときだけ `f32`、
それ以外は `f64` です(§7.2)。整数 literal は float になりません。

`print` と `std::testing::expect_equal` は float を受け付けません。文字列への
変換は `std` が持ちます。wasm32 target は f32 / f64 を含む program を build 時に
target 非対応として拒否します。
...
literal の一部ではなく単項演算子です。float literal の形は §6.9.3 にあります。
...
整数の演算と overflow の意味は §6.9.2、浮動小数点の演算と cast は §6.9.3 に
あります。
...
被演算子(§6.9.2)など、期待する整数型が一意に決まる場所です。float literal も
同じ場所で `f32` として扱えます。それ以外では `f64` です。
